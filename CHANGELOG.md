...
### Fixed
- **A judge verdict now binds to the code it reviewed, or it does not count**
  (`enforcement/validators/task_state.py`, `enforcement/ci/pre-push`). `_judge_snapshot` binds a verdict to the
  patch the judge reviewed, but an empty in-scope diff hashed nothing and returned `sha256("")` — a
  valid-looking digest that *every* empty diff reproduces. Two live consequences: (1) a judge invoked with an
  empty index recorded a `pass` bound to no code and the gate accepted it; (2) the local push gate and CI
  disagreed about the same commit, because after a commit the local index is empty while CI recomputes over
  `VEMO_DIFF_RANGE=<before>...HEAD`. Now only a **64-hex content digest binds a verdict**
  (`_binds_to_content`): the empty-diff sentinel, `unresolved-range`, `invalid-range`, a git failure (`""`)
  and legacy rows with no `snapshot` key are all refused — at write time (`judge_record` returns `refused:`
  and exits non-zero) and again at gate time, so a row injected straight into the log is still rejected. A
  `fail` remains recordable with no staged diff, since a judge may fail a task precisely because the change
  is absent. `enforcement/ci/pre-push` now hands the pushed range it already derives to the validator, so the
  local gate stops evaluating an empty index. When Git supplies refs on stdin, that current push range also
  overrides any stale inherited `VEMO_DIFF_RANGE`; only CI's stdin-free invocation keeps the environment range,
  preventing an older reviewed snapshot from being replayed to accept a newer push. A push carrying more than
  one non-deletion ref now fails closed with `multi-ref-push-unsupported` and asks for one ref at a time: the
  scalar snapshot contract cannot honestly prove several independently moving ranges, and checking only the
  first let its passes authorize an unreviewed change on the second. For a new single ref, the hook now derives
  the full outgoing range from one known target-remote boundary; it fails closed when that boundary is missing
  or ambiguous instead of checking only the tip commit and omitting earlier unreviewed history. Existing-ref
  pushes and push-event CI use the exact two-tree range (`remote..local` / `before..HEAD`), so a non-fast-forward
  update cannot omit remote-only files that it deletes; an all-zero initial CI baseline uses Git's empty tree.
  **Why a whitelist:** the first attempt blacklisted the empty hash, and an independent governance-judge pass
  recorded FAIL demonstrating that the same class stayed open through the other sentinels — they are
  *self-matching* strings, so recording and re-reading both yield the same value and an equality check sees a
  match. Two `pass` rows against a nonexistent commit range opened the R2 gate. The same review found one of
  the new tests was **vacuous** (deleting the clause it claimed to cover left the suite green). A redundant
  re-check inside the pass-counting loop was removed rather than tested: the guard above it proves the current
  snapshot binds and equality then forces the recorded one to bind, so the check was unreachable by
  construction, and an unreachable check reads as coverage it cannot provide.
  Regression-covered by `tests/test_judge_snapshot.py` (13 tests; the two range-bypass tests fail against the
  old blacklist) plus an **executed** conformance scenario driving `pre-push` through git's stdin contract in
  a real repository, including stale-environment, multi-ref, new-branch-history, and non-fast-forward replay
  attempts — a grep of the script cannot see a local/CI divergence or range-selection bug. Six existing conformance
  scenarios that recorded passes in git-less sandboxes were given real reviewable diffs, so judge-depth
  requirements are still proven rather than relaxed.
  **Honest coverage note:** `.vemo/judge.jsonl` is append-only, so the 12 pre-existing rows carrying the empty
  hash (across `T-20260724-…readonly-probe-ex`, `T-20260908-wildmeerkat-…`, `T-20260909-ignore-nested-…`) and
  the 28 older rows with no `snapshot` key at all are NOT rewritten — they are rejected on read. Those tasks'
  local gates were opened by snapshots that bound to nothing, so their recorded passes are weaker evidence
  than their task files claim, and their merged code was never re-judged under this rule. New multi-ref pushes
  are rejected rather than being represented by the first range alone.
