...
> machine-readable so CI can check it.
...
4. **Silent takeover is forbidden** — and a pre-commit check rejects a commit whose task `owning_chat` ≠ the committing session unless a logged takeover exists.
...
VEMO keeps the human-visible takeover prompt and adds a pre-commit check that the committer owns the task, so a
forgetful or misaligned session is stopped by mechanism, not etiquette.
