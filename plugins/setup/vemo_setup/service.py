...
INSTALL_FILES = REQUIRED_FILES | {"CLAUDE.md", ".gitignore", ".claude/settings.json",
                                "vemo.task.json", ".git/hooks/pre-commit",
...
def managed_path(relative):
    special = {"CLAUDE.md", ".gitignore", ".claude/settings.json", "vemo.task.json",
...
def _desired(source, root, preset):
    ...
    desired["vemo.task.json"] = desired["vemo.task.example.json"]
...
def plan_install(source, target, preset="python", profile="solo"):
    ...
    actions, conflicts = [], []
    if safe_path(root, "vemo.config.yaml").exists():
        conflicts.append("检测到 1.x vemo.config.yaml；请先显式映射 critical_paths 与 verification 后再安装")
...
def _python_probes(root, hashes):
    """Run only the installed minimal contract in a private import tree."""
    with tempfile.TemporaryDirectory(prefix="vemo-setup-probes-") as temporary:
        runtime = Path(temporary)
        for relative, expected_hash in hashes.items():
            if relative in REQUIRED_FILES or relative == "vemo.task.json":
        ...
        for name, target, expected in (("Guard allow live-fire", "src/probe.py", 0),
                                       ("Guard deny live-fire", "outside.py", 2)):
            request = json.dumps({"tool_name": "Write", "tool_input": {"file_path": target, "content": "probe\n"}})
            result = run([sys.executable, "-I", "-S", "-B", str(dispatcher), "edit"], runtime, input=request)
            yield {"name": name, "kind": "live_fire", "ok": result.returncode == expected,
...
def check_install(target, expected=None):
    ...
    return {"target": str(root), "ready": all(row["ok"] for row in checks), "checks": checks,
            "next": ["在宿主中信任项目并重启会话，确认实际 hooks 被加载。",
                     ...
                     "检查 vemo.json 与 vemo.task.json，再开始任务并运行 vemo verify。"]}
