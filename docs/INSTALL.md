...
安装还携带 CI 所需的 `eval/run.py`、框架测试、文档及资源。框架测试放在 `eval/tests/`，与项目自己的 `tests/` 分开；CI 的框架检查只运行这份测试集。已有 README 通过 VEMO 标记块追加框架说明，卸载时恢复原文；其他文档或文件同名且内容不同时，会显示冲突供审查。不会携带源码仓库的历史任务或 `eval/out/` 运行结果。
...
任务、验收日志、judge 记录、业务代码以及空目录保留。已有项目说明与设置恢复安装前内容。没有 `.vemo/install.json` 的历史手动安装不会被自动卸载。
